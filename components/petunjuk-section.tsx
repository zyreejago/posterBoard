...
interface PetunjukSectionProps {
  onBack: () => void
}

export default function PetunjukSection({ onBack }: PetunjukSectionProps) {
  ...
  const instructions = [
    ...
    },
    {
      ...
      content:
        "Pada menu utama, kamu dapat memilih salah satu dari empat pilihan: Materi, Poster, Quiz, atau Petunjuk.",
    },
    {
      title: "Materi",
    ...
    },
    {
      ...
      content:
        "Lihat poster tentang persatuan di lingkungan sekolah yang berisi pengertian, pentingnya persatuan, contoh, dan tindakan yang tidak mencerminkan persatuan.",
    },
    {
      title: "Quiz",
      icon: <HelpCircle size={24} className="text-orange-500" />,
      content:
        "Klik untuk membuka quiz interaktif di platform Zep.us. Quiz ini akan menguji pemahamanmu tentang persatuan di lingkungan sekolah.",
    },
  ]
          ...
          {instructions.map((instruction, index) => (
            <motion.div
              ...
              className={`flex gap-4 p-4 rounded-lg ${
                activeInstruction === index ? "bg-blue-100 border-l-4 border-blue-500" : "bg-blue-50"
              }`}
